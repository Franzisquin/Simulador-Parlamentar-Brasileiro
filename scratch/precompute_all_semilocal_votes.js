...
async function run() {
  ...
  for (const year of years) {
    ...
    for (const uf of states) {
      ...
      const processResultsFile = async (resultsZipPath, resJsonName, electionKey) => {
        if (!fs.existsSync(resultsZipPath)) return;
        ...
        const resultJson = JSON.parse(await resEntry.text());
        const candNames = resultJson.METADATA.cand_names || {};
      ...
      // 1. Process Federal Deputy (Deputado Federal) Votes
      const depFedZipPath = path.resolve(DATA_BASE_URL, `Legislativas ${year}/deputados_federal_${year}_${uf}.zip`);
      await processResultsFile(depFedZipPath, `deputados_federal_${year}_${uf}.json`, `deputado_${year}`);
      process.stdout.write(` DepFed`);

      // 2. Process President (Presidente) Votes
      const presZipPath = path.resolve(DATA_BASE_URL, `Majoritarias ${year}/presidente_${year}_t1_${uf}.zip`);
      await processResultsFile(presZipPath, `presidente_${year}_t1_${uf}.json`, `presidente_${year}`);
      process.stdout.write(` Pres`);

      // 3. Process State Deputy (Deputado Estadual) Votes
      const depEstZipPath = path.join(OBS_BASE_URL, `Legislativas ${year}/deputados_estadual_${year}_${uf}.zip`);
      await processResultsFile(depEstZipPath, `deputados_estadual_${year}_${uf}.json`, `estadual_${year}`);
      process.stdout.write(` DepEst`);

      // 4. Process Governor (Governador) Votes
      let govZipPath = '';
      let govJsonName = '';
      if (year === 2022 || year === 2018) {
        govZipPath = path.join(OBS_BASE_URL, `Majoritarias ${year}/governador_${year}_t1_${uf}.zip`);
        govJsonName = `governador_${year}_t1_${uf}.json`;
      } else {
        govZipPath = path.join(OBS_BASE_URL, `Majoritarias ${year}/governador_${year}_ord_t1_${uf}.zip`);
        govJsonName = `governador_${year}_ord_t1_${uf}.json`;
      }
      await processResultsFile(govZipPath, govJsonName, `governador_${year}`);
      process.stdout.write(` Gov`);

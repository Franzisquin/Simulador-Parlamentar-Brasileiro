...
function getStandardFederationKey(name, year) {
  ...
  if (year >= 2022) {
    if (partyKey === 'PT' || partyKey === 'PV' || partyKey === 'PCDOB') return 'FE_BRASIL';
    if (partyKey === 'PSDB' || partyKey === 'CIDADANIA') return 'PSDB_CIDADANIA';
    if (partyKey === 'PSOL' || partyKey === 'REDE') return 'PSOL_REDE';
  }
  return partyKey;
}

// ---- Main ----
// We will store: fid -> electionKey -> muniName -> party -> votes
const distritosVotos = {}; 

async function run() {
  console.log('=== Pre-computing Detailed Municipal Votes inside Districts ===\n');
  ...
  for (const year of years) {
    ...
    for (const uf of states) {
      ...
      // Map locations to districts and municipalities
      // key: locationKey, value: { fid, muniName }
      ...
      for (const feat of locGeo.features) {
        ...
        const rawMuni = props.NM_LOCALIDADE || props.nm_localidade || props.NM_MUNICIPIO || props.nm_municipio || "UNKNOWN";
        const muniName = String(rawMuni).toUpperCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
        ...
        if (container) {
          locationMapping[key] = { fid: container.fid, muniName };
        }
      }

      for (const type of ['presidente']) {
        const electionKey = `${type}_${year}`;
        
        const resultsZipPath = path.resolve(DATA_BASE_URL, `Majoritarias ${year}/presidente_${year}_t1_${uf}.zip`);
        const resJsonName = `presidente_${year}_t1_${uf}.json`;

        if (!fs.existsSync(resultsZipPath)) continue;
        ...
        const resultsJson = JSON.parse(await resEntry.text());
        const candNames = resultsJson.METADATA.cand_names || {};
        ...
        for (const [locKey, voteMap] of Object.entries(results)) {
          ...
          if (!distritosVotos[districtFid][electionKey][muniName]) {
              distritosVotos[districtFid][electionKey][muniName] = {};
          }

          const distMap = distritosVotos[districtFid][electionKey][muniName];
  ...
  const outputPath = path.resolve(DATA_BASE_URL, 'distritos_detalhe_munis.json');
  fs.writeFileSync(outputPath, JSON.stringify(distritosVotos));
  const sizeMB = (fs.statSync(outputPath).size / (1024 * 1024)).toFixed(1);
  console.log(`\nSuccessfully exported detailed municipal votes to ${outputPath} (${sizeMB} MB)`);
}
